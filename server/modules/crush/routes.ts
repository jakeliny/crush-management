...
class CrushRoutes {
  ...
  create(req, res) {
    ...
    CrushController.create(crush)
      .then(crush =>
        sendResponse(res, httpStatus.CREATED, 'Crush criado com amor!'),
      )
      .catch(err => {
        console.error('Erro: ' + err);
        sendResponse(res, httpStatus.INTERNAL_SERVER_ERROR, {});
  ...
  update(req, res) {
    const id = { _id: req.params.id };
    const crush = req.body;
    CrushController.update(id, crush)
